...
// CUDA核函数：使用双线性插值进行仿射变换
__global__ void warp_affine_bilinear_kernel(const uint8_t* src, const int src_cols, const int src_rows,
    ...
    // 获取四个点的坐标
    int src_x0 = __float2int_rd(src_xy.x); // * 代表公式中的 x1
    int src_y0 = __float2int_rd(src_xy.y); // * 代表公式中的 y1
    int src_x1 = src_x0 + 1;               // * 代表公式中的 x2
    int src_y1 = src_y0 + 1;               // * 代表公式中的 y2
    // * (x2 - x1)(y2 - y1) = 1
    ...
    // 提前计算好差值
    float wx0 = src_x1 - src_xy.x; // * 代表公式中的 (x2 - x)
    float wx1 = src_xy.x - src_x0; // * 代表公式中的 (x - x1)
    float wy0 = src_y1 - src_xy.y; // * 代表公式中的 (y2 - y)
    float wy1 = src_xy.y - src_y0; // * 代表公式中的 (y - y1)

    // 计算四个 f * wx * wy 的值并求和
    float3 border_value = make_float3(114.0f, 114.0f, 114.0f); // 默认填充值
    uchar3* input        = (uchar3*)((uint8_t*)src + src_y0 * src_cols * 3);
    src_value0           = flag0 ? uchar3_to_float3(input[src_x0]) : border_value; // * 代表公式中的 f(Q_11)
    src_value1           = flag1 ? uchar3_to_float3(input[src_x1]) : border_value; // * 代表公式中的 f(Q_21)
    value0               = wx0 * wy0 * src_value0; // * 代表公式中的 f(Q_11)(x2 - x)(y2 - y)
    value1               = wx1 * wy0 * src_value1; // * 代表公式中的 f(Q_21)(x - x1)(y2 - y)
    float3 sum           = value0 + value1;

    input       = (uchar3*)((uint8_t*)src + src_y1 * src_cols * 3);
    src_value0  = flag2 ? uchar3_to_float3(input[src_x0]) : border_value; // * 代表公式中的 f(Q_12)
    src_value1  = flag3 ? uchar3_to_float3(input[src_x1]) : border_value; // * 代表公式中的 f(Q_22)
    value0      = wx0 * wy1 * src_value0; // * 代表公式中的 f(Q_12)(x2 - x)(y - y1)
    value1      = wx1 * wy1 * src_value1; // * 代表公式中的 f(Q_22)(x - x1)(y - y1)
    sum        += value0 + value1;
...
int main() {
    ...
    uint8_t* d_src;
    float* d_dst;
    CHECK_CUDA(cudaMalloc((void**)&d_src, src_width * src_height * sizeof(uint8_t) * 3));
    CHECK_CUDA(cudaMalloc((void**)&d_dst, dst_width * dst_height * sizeof(float) * 3));

    // 将OpenCV图像转换为uchar3格式并复制到设备内存
    uint8_t* h_src = (uint8_t*)src_image.data;
    CHECK_CUDA(cudaMemcpy(d_src, h_src, src_width * src_height * sizeof(uint8_t) * 3, cudaMemcpyHostToDevice));
    ...
    // 启动CUDA核函数
    warp_affine_bilinear_kernel<<<grid, block>>>(d_src, src_width, src_height, d_dst, dst_width, dst_height, m0, m1);

    // 检查核函数是否成功启动
    CHECK_CUDA(cudaGetLastError());

    // 分配主机内存用于存储目标图像
    float* h_dst = (float*)malloc(dst_width * dst_height * sizeof(float) * 3);
    CHECK_CUDA(cudaMemcpy(h_dst, d_dst, dst_width * dst_height * sizeof(float) * 3, cudaMemcpyDeviceToHost));
