...
{
    // ! 无论选择哪种边界框编码方式，IOU始终在 BoxCorner 编码中进行。
    // 复制边界框，以便可以重新排序而不影响原始边界框。
    BoxCorner<T> b1 = box1; // 复制 box1
    BoxCorner<T> b2 = box2; // 复制 box2
    b1.reorder(); // 重新排序 box1 的坐标
    b2.reorder(); // 重新排序 box2 的坐标
    float intersectArea = BoxCorner<T>::intersect(b1, b2).area(); // 计算交集面积
    // 如果交集面积小于等于 0，返回 0
    if (intersectArea <= 0.f)
    {
        return 0.f;
    }
    float unionArea = b1.area() + b2.area() - intersectArea; // 计算并集面积
    // 如果并集面积小于等于 0，返回 0
    if (unionArea <= 0.f)
    {
        return 0.f;
...
{
    // ! 输入将使用选定的编码格式，解码后的边界框将始终以 BoxCorner 格式返回
    Tb box = boxesInput[boxIdx];             // 获取当前边界框
    // 如果不需要解码，直接返回 BoxCorner 格式的边界框
    if (!param.boxDecoder)
    {
    ...
    return BoxCorner<T>(box.decode(anchor)); // 解码边界框并返回
}

// * 与 MapRotatedNMSData 完全一致
// 映射 NMS 数据
template <typename T, typename Tb>
...
// * 与 WriteRotatedNMSResult 基本一致
// * 改动：参考 WriteONNXResult，新增 boxIdxMap 参数
// 写入 NMS 结果
template <typename T>
    ...
    unsigned int resultsCounter, int boxIdxMap)
{
    ...
    // * 新增：用于获取索引
    int index = boxIdxMap % param.numAnchors;                             // 计算索引
    nmsIndicesOutput[outputIdx] = index;                                  // 写入索引
}

// * 与 EfficientRotatedNMS 完全一致
// * 改动：删除 ONNX 插件相关部分
// EfficientNMS CUDA 内核函数
template <typename T, typename Tb>
    ...
    int* __restrict__ nmsClassesOutput, int* __restrict__ nmsIndicesOutput, BoxCorner<T>* __restrict__ nmsBoxesOutput)
{
    ...
    if (thread >= numSelectedBoxes)                                           // 如果线程索引超出选定的边界框数量，直接返回
    {
        return;
    }
    ...
    BoxCorner<T> threadBox[NMS_TILES];      // 每个 Tile 的边界框
...
// * 与 EfficientRotatedNMSLauncher 完全一致
// * 改动：删除 ONNX 插件相关部分
// EfficientNMS 启动函数
template <typename T>
    ...
    int* nmsClassesOutput, int* nmsIndicesOutput, void* nmsBoxesOutput, cudaStream_t stream)
{
    ...
    if (param.boxCoding == 0) // 如果边界框编码为 BoxCorner
    {
    ...
    else if (param.boxCoding == 1) // 如果边界框编码为 BoxCenterSize
    {
...
// * 与 EfficientRotatedNMSFilterSegments 完全一致
...
// * 与 EfficientRotatedNMSFilter 完全一致
// EfficientNMS 过滤的内核函数
template <typename T>
...
// * 与 EfficientRotatedNMSDenseIndex 完全一致
// EfficientNMS 密集索引的内核函数
template <typename T>
...
// * 与 EfficientRotatedNMSFilterLauncher 完全一致
// EfficientNMS 过滤启动函数
template <typename T>
...
// * 与 EfficientRotatedNMSSortWorkspaceSize 完全一致
// 计算 EfficientNMS 排序工作空间大小
template <typename T>
...
// * 与 EfficientRotatedNMSWorkspaceSize 完全一致
// 计算 EfficientIdxNMS 工作空间大小
...
// * 与 EfficientRotatedNMSWorkspace 完全一致
// 获取 EfficientNMS 工作空间
template <typename T>
...
// * 改动：删除 ONNX 插件相关部分
// * 与 EfficientRotatedNMSDispatch 完全一致
// EfficientNMS 调度函数
template <typename T>
...
// * 与 EfficientRotatedNMSInference 基本一致
// EfficientIdxNMS 推理函数
