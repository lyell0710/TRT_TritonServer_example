    ...
    int32_t index, nvinfer1::DataType const* inputTypes, int32_t nbInputs) const noexcept
{
    ...
    if (index == 0 || index == 3 || index == 4)  // * 新增 index == 4 代表 detection_indices
    {
    ...
    int32_t outputIndex, DimsExprs const* inputs, int32_t nbInputs, IExprBuilder& exprBuilder) noexcept
{
    try
    {
        ...
        else if (outputIndex == 2 || outputIndex == 3 || outputIndex == 4)  // * 新增 index == 4 代表 detection_indices
        {
            out_dim.nbDims = 2;
            out_dim.d[0] = inputs[0].d[0];
            out_dim.d[1] = numOutputBoxes;
    ...
    int32_t pos, PluginTensorDesc const* inOut, int32_t nbInputs, int32_t nbOutputs) noexcept
{
    ...
    PLUGIN_ASSERT(nbOutputs == 5); // * 4 -> 5
    if (nbInputs == 2)
    {
        PLUGIN_ASSERT(0 <= pos && pos <= 6); // * 5 -> 6
    }
    if (nbInputs == 3)
    {
        PLUGIN_ASSERT(0 <= pos && pos <= 7); // * 6 -> 7
    }
    ...
    void const* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream) noexcept
{
    try
    {
        ...
        void* nmsIndicesOutput = outputs[4];  // * 新增 nmsIndicesOutput 用于获取索引
