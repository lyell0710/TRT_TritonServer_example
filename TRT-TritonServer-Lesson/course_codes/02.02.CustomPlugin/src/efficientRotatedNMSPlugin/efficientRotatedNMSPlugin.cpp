...
int32_t EfficientRotatedNMSPlugin::initialize() noexcept
{
    if (!initialized)
    {
        ...
        {
            // Most Devices
            mParam.numSelectedBoxes = 2000;  // * 5000 -> 2000，防止显存不够
        }
        else
        {
            // Jetson TX1/TX2
            mParam.numSelectedBoxes = 1000;  // * 2000 -> 1000，防止显存不够
        }
        initialized = true;
    }
...
// * 改动：1. 删除 ONNX 插件相关部分
// * 改动：2. boxes shape: [batch_size, num_boxes, 4] -> [batch_size, num_boxes, 5] 相关修改
    ...
    int32_t outputIndex, DimsExprs const* inputs, int32_t nbInputs, IExprBuilder& exprBuilder) noexcept
{
    try
    {
        ...
        else if (outputIndex == 1)
        {
            ...
            out_dim.d[2] = exprBuilder.constant(5);  // * 4 -> 5， OBB 的 boxes 多了一个角度信息
        }
    ...
    DynamicPluginTensorDesc const* in, int32_t nbInputs, DynamicPluginTensorDesc const* out, int32_t nbOutputs) noexcept
{
    try
    {
        ...
        {
            PLUGIN_ASSERT(in[0].desc.dims.d[2] == 5);  // * 4 -> 5， OBB 的 boxes 多了一个角度信息
        ...
        else
        {
            ...
            PLUGIN_ASSERT(in[0].desc.dims.d[3] == 5); // * 4 -> 5， OBB 的 boxes 多了一个角度信息
