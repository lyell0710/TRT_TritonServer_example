...
```bash
/usr/src/tensorrt/bin/trtexec --onnx=models/yolo11n_with_plugin.onnx --saveEngine=models/yolo11n_with_plugin.engine --fp16
```
...
3. 使用以下命令运行推理：
    ...
    ```bash
    cd bin
    ./detect -e ../models/yolo11n_with_plugin.engine -i ../images -o ../output -l ../labels.txt
    ```
