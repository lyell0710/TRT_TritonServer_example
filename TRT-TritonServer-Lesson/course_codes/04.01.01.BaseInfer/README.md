...
- `backend.hpp` 与 `backend.cpp` 新增 `std::unique_ptr<InferBackend> InferBackend::clone()` 方法，用于多上下文推理

- `manager.hpp` 与 `manager.cpp` 新增 `void TRTManager::loadPluginLibrary(const std::string& path)` 方法，用于全局加载插件动态库。