...
int main(int argc, char** argv) {
    try {
        ...
        auto model  = std::make_unique<deploy::OBBModel>(engine_path);
        auto model2 = std::make_unique<deploy::OBBModel>(engine_path);
        auto model3 = std::move(model);  // 使用移动赋值操作符

        // 检查 model 是否为空
        if (!model) {
            std::cout << "model is now empty after move." << std::endl;
        } else {
            std::cout << "model is still valid (this should not happen!)." << std::endl;
        }

        if (fs::is_regular_file(input_path)) {
            process_single_image(input_path, output_path, *model3, labels);
            // process_single_image(input_path, output_path, *model2, labels);
        } else {
            auto image_files = get_images_in_directory(input_path);
            if (image_files.empty()) {
                throw std::runtime_error("Failed to read image from path: " + input_path);
            }
            process_batch_images(image_files, output_path, *model3, labels);
            // process_batch_images(image_files, output_path, *model2, labels);
        }
