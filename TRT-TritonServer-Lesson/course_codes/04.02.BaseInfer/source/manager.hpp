...
public:
     ...
     * @brief 实例化 CUDA 图
     *
     * @param stream 要实例化的 CUDA 流。
     */
    void instantiate(cudaStream_t stream);

    /**
     * @brief 执行 CUDA 图
     *
     * @param stream 要执行的 CUDA 流。
     */
     ...
     */
    void getNodesParams(size_t num);
     ...
     */
    void setKernelNodeParams(size_t index, void** params);
     ...
     */
    void setMemcpyNodeParams(size_t index, void* src, void* dst, size_t size);
...
private:
    ...
    cudaGraph_t                        graph_;       // 图句柄，默认初始化为 nullptr
    cudaGraphExec_t                    graph_exec_;  // 执行图句柄，默认初始化为 nullptr
};
